...
function addScore() {
    ...
    let select = document.getElementById("attackSelect").value;
    ...
    if (currentPlayer === "player1") {
        ...
        if (select === "lightAttack") {
            player1Score -= Math.floor(Math.random() * 4) + 1;
        } else if (select === "heavyAttack") {
            player1Score -= Math.floor(Math.random() * 14);
        } else {
            player2Score += Math.floor(Math.random() * 3) + 1; // add score to player 2
        }
